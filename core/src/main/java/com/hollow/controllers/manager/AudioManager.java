...
import com.hollow.loader.AudioLoader;
...
public class AudioManager {
    ...
    public void stopMusic() {
        isFading = false;
        safeStop(currentMusic, prevMusic, currentAtmos, currentMainLayer,
            currentBassLayer, prevAtmos, prevMainLayer, prevBassLayer);
        currentMusic = currentAtmos = currentMainLayer = currentBassLayer = null;
        prevMusic = prevAtmos = prevMainLayer = prevBassLayer = null;
    }

    public void pauseMusic() {
        safePause(currentMusic, prevMusic, currentAtmos, currentMainLayer,
            currentBassLayer, prevAtmos, prevMainLayer, prevBassLayer);
    }
    ...
    public void playFootsteps(boolean isGrass) {
        ...
        Sound targetSound = isGrass ? audioLoader.knightRunGrass : audioLoader.knightRunStone;
    ...
    public void playFallingSound() {
        ...
        if (audioLoader.knightFalling != null) {
            fallingSoundId = audioLoader.knightFalling.loop(0.8f);
            isFallingSoundPlaying = true;
        }
    }

    public void stopFallingSound() {
        if (isFallingSoundPlaying && audioLoader.knightFalling != null && fallingSoundId != -1) {
            audioLoader.knightFalling.stop(fallingSoundId);
        }
        fallingSoundId = -1;
    ...
    public void playCharmProximityLoop() {
        ...
        if (audioLoader.knightPickUpSpell != null) {
            charmLoopId = audioLoader.knightPickUpSpell.loop(0.6f);
            isCharmLoopPlaying = true;
        }
    }

    public void stopCharmProximityLoop() {
        if (isCharmLoopPlaying && audioLoader.knightPickUpSpell != null && charmLoopId != -1) {
            audioLoader.knightPickUpSpell.stop(charmLoopId);
        }
        charmLoopId = -1;
    ...
    public void playWallSlideLoop() {
        ...
        if (audioLoader.knightSlidingWall != null) {
            wallSlideSoundId = audioLoader.knightSlidingWall.loop(0.7f);
            isWallSlidePlaying = true;
        }
    }

    public void stopWallSlideLoop() {
        if (isWallSlidePlaying && audioLoader.knightSlidingWall != null && wallSlideSoundId != -1) {
            audioLoader.knightSlidingWall.stop(wallSlideSoundId);
        }
        wallSlideSoundId = -1;
    ...
    public void playFocusChargeLoop() {
        ...
        if (audioLoader.knightFocusCharge != null) {
            focusChargeSoundId = audioLoader.knightFocusCharge.loop(0.9f);
            isFocusChargePlaying = true;
        }
    }

    public void stopFocusChargeLoop() {
        if (isFocusChargePlaying && audioLoader.knightFocusCharge != null && focusChargeSoundId != -1) {
            audioLoader.knightFocusCharge.stop(focusChargeSoundId);
        }
    ...
    public void playFalseKnightShout() {
        if (!isSfxOn()) return;
        if (audioLoader.fkShouts != null && audioLoader.fkShouts.length > 0) {
            int index = MathUtils.random(0, audioLoader.fkShouts.length - 1);
            if (audioLoader.fkShouts[index] != null) audioLoader.fkShouts[index].play(0.9f);
        }
    }

    public void playWallHitSound() {
        if (!isSfxOn()) return;
        Sound[] hits = {audioLoader.wallHit1, audioLoader.wallHit2};
    ...
    public void playWallDeathSound() {
        if (!isSfxOn()) return;
        if (audioLoader.wallDeath != null) audioLoader.wallDeath.play(0.9f);
    }
