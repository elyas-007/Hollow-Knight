...
public class Crystallized extends Enemy {
    ...
    private void handleState(float delta) {
        ...
        switch (state) {
            ...
            case DYING_AIR -> position.x += velocity.x * delta;
        }
    }
    ...
    public TextureRegion getCurrentFrame() {
        ...
        if (state == EnemyState.EVADING && evadeAnim != null)
            return evadeAnim.getKeyFrame(stateTime, false);
        if (state == EnemyState.ATTACK_ANTICIPATE && shootAnim != null)
            return shootAnim.getKeyFrame(stateTime, false);
        if (state == EnemyState.ATTACK_LUNGE && runAnim != null)
