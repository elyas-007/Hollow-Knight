...
public class FalseKnight {
    ...
    public enum State {
        IDLE, MACE_SLAM, CHARGE_RUN, OFFENSIVE_LEAP, DEFENSIVE_LEAP, POWER_SLAM, STUNNED, DEATH
    }
    ...
    public float leapVelocityY = 30f;
    ...
    public void takeDamage(int damage) {
        ...
        if (currentHp <= 0) {
            currentHp = 0;
            if (audioManager != null) audioManager.playSound(audioManager.audioLoader.fkDeath);
        ...
        if (currentHp <= maxHp / 2 && !isPhaseTwo) {
            isPhaseTwo = true;
            if (audioManager != null) audioManager.playSound(audioManager.audioLoader.fkStun);
    ...
    public TextureRegion getCurrentFrame(float delta) {
        ...
        switch (currentState) {
            case IDLE -> { return idleAnim.getKeyFrame(animationStateTime, true); }
            case CHARGE_RUN -> {
                if (!runAnticAnim.isAnimationFinished(animationStateTime)) return runAnticAnim.getKeyFrame(animationStateTime);
                float runTime = animationStateTime - runAnticAnim.getAnimationDuration();
                return runAnim.getKeyFrame(runTime, true);
            }
            case MACE_SLAM -> {
                if (!maceSlamAnticAnim.isAnimationFinished(animationStateTime)) return maceSlamAnticAnim.getKeyFrame(animationStateTime);
                float afterAntic = animationStateTime - maceSlamAnticAnim.getAnimationDuration();
                if (!maceSlamAnim.isAnimationFinished(afterAntic)) return maceSlamAnim.getKeyFrame(afterAntic);
            ...
            case OFFENSIVE_LEAP, DEFENSIVE_LEAP -> {
                if (!jumpAnticAnim.isAnimationFinished(animationStateTime)) return jumpAnticAnim.getKeyFrame(animationStateTime);
                float airTime = animationStateTime - jumpAnticAnim.getAnimationDuration();
                return jumpAnim.getKeyFrame(airTime, true);
            }
            case POWER_SLAM -> {
                if (!jumpAttackAnim.isAnimationFinished(animationStateTime)) return jumpAttackAnim.getKeyFrame(animationStateTime);
            ...
            case STUNNED -> { return stunAnim.getKeyFrame(animationStateTime, true); }
            case DEATH -> {
                if (!deathFallAnim.isAnimationFinished(animationStateTime)) return deathFallAnim.getKeyFrame(animationStateTime);
                float afterFall = animationStateTime - deathFallAnim.getAnimationDuration();
                if (!deathHitAnim.isAnimationFinished(afterFall)) return deathHitAnim.getKeyFrame(afterFall);
